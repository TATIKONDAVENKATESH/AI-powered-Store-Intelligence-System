...
def build_event(
    ...
    """Build a single structured event dict matching the required schema."""
    return {
...
class EventEmitter:
    ...
    def flush(self, output_path: Optional[str] = None) -> str:
        """Write buffered events to a JSONL file and return the path."""
        os.makedirs(EVENTS_DIR, exist_ok=True)
        if output_path is None:
            output_path = os.path.join(
                EVENTS_DIR, f"{self.camera_id}_events.jsonl"
            )
...
def merge_event_files(output_path: str) -> int:
    """Merge all per-camera JSONL files in EVENTS_DIR into one sorted file."""
    ...
    # Sort by timestamp so ingestion order is chronological
